...
1. **P1: The stronger secret walker has no lasting regression test**
   ...
   Evidence: The plan admits the existing tests remain green with the weak walker because no current exception carries the sentinel through an attribute ([plan, lines 59-62](docs/plans/2026-09-24-test-helpers-home.md#L59)). Verification proposes only a temporary mutation ([lines 194-200](docs/plans/2026-09-24-test-helpers-home.md#L194)). Consequently, reverting `chain` later to the old `repr`/`str` walk would leave the committed suite green. The behavior that matters, including the PyYAML-like second-level attribute described by [`config_cli.py`, lines 367-381](vinga-server/tests/support/config_cli.py#L367), is not pinned.
...
2. **P1: The migration module leaves Alembic configuration duplicated and would strand downgrade callers**
   ...
   Evidence: The proposed interface exposes only blank-database upgrade and version reads ([plan, lines 111-136](docs/plans/2026-09-24-test-helpers-home.md#L111)). However, `test_metrics_views_upgrade.py` builds the same Alembic configuration in `_alembic` ([lines 103-113](vinga-server/tests/integration/test_metrics_views_upgrade.py#L103)) and still needs it for two downgrades ([lines 407-420](vinga-server/tests/integration/test_metrics_views_upgrade.py#L407), [lines 509-515](vinga-server/tests/integration/test_metrics_views_upgrade.py#L509)). Deleting `_alembic` breaks those tests; retaining it leaves the three Alembic requirements in two places, contradicting the module’s stated locality benefit.
...
3. **P2: The plan defers resolvable exception-walker decisions to implementation**
   ...
   Evidence: The plan leaves seven walkers to conditional disposition in the implementation document ([lines 99-109](docs/plans/2026-09-24-test-helpers-home.md#L99)). The current tree already resolves most of them: both `_whole_chain` implementations inspect only text and arguments ([provider boundary, lines 678-689](vinga-server/tests/unit/test_providers_boundary.py#L678), [reach upgrade, lines 404-416](vinga-server/tests/integration/test_reach_upgrade.py#L404)); `carried` performs the same attribute walk less completely ([CLI live, lines 411-438](vinga-server/tests/integration/test_cli_live.py#L411)); and `turntaking.chained` is another weaker secret-absence walk ([lines 120-130](vinga-server/tests/unit/test_turntaking.py#L120)). `Consumer.rendered` cannot be replaced wholesale, but its exception traversal can delegate to `leaks.chain` ([server event pins, lines 99-114](vinga-server/tests/unit/test_server_event_pins.py#L99)).
...
4. **P2: The residual duplicate inventory omits meaningful multi-file groups**
   ...
   Evidence: The plan characterizes everything else as three lines or fewer or unrelated two-file pairs ([lines 184-186](docs/plans/2026-09-24-test-helpers-home.md#L184)). That does not cover the identical five-file `out(run, capsys, ...)` helper, for example [`test_config_cli_sessions.py`, lines 145-149](vinga-server/tests/unit/test_config_cli_sessions.py#L145), nor the identical ordered conversation-row reader in three files ([namespace, lines 191-202](vinga-server/tests/unit/test_conversations_namespace.py#L191), [retention, lines 202-213](vinga-server/tests/unit/test_conversations_retention.py#L202), [erasure, lines 194-205](vinga-server/tests/unit/test_conversations_erasure.py#L194)). Both groups share domain meaning, not merely syntax.
...
5. **P2: The stdio helpers cannot pass the plan’s required AST-identity check**
   ...
   Evidence: The plan requires every moved body to be AST-identical to its destination and says divergent copies are not silently unified ([lines 79-83](docs/plans/2026-09-24-test-helpers-home.md#L79)), then calls all listed stdio helpers identical ([lines 146-155](docs/plans/2026-09-24-test-helpers-home.md#L146)). `test_mcp_reload.stdio_entry` has no override parameter or dictionary union ([lines 41-46](vinga-server/tests/integration/test_mcp_reload.py#L41)), while `test_tools_mcp_reload.stdio_entry` calls its local `entry_data` and carries a docstring ([lines 83-94](vinga-server/tests/unit/test_tools_mcp_reload.py#L83)). Neither body is AST-identical to the support destination.
...
6. **P2: The smoke-copy exception rests on a false isolation claim**
   ...
   Evidence: The plan says the smoke lane imports nothing except its own conftest and therefore must retain `speech_pcm` ([lines 138-144](docs/plans/2026-09-24-test-helpers-home.md#L138)). In fact, the smoke conftest imports `vinga_server.auth` and `vinga_server.config.models` ([lines 53-67](vinga-server/tests/smoke/conftest.py#L53)), and CI installs the full contributor environment before running smoke tests from the checkout ([workflow, lines 1237-1252](.github/workflows/vinga-server.yml#L1237), [lines 1646-1655](.github/workflows/vinga-server.yml#L1646)). Importing a client-side waveform generator does not make the server cease to be the external container.
...
7. **P2: `llm_sdk.py` does not own the proposed ASR/TTS helper as documented**
   ...
   Evidence: The plan places the shared `AsyncOpenAI` client constructor in `tests/support/llm_sdk.py` and says only `leaks.py` needs documentation changes ([plan, lines 157-165](docs/plans/2026-09-24-test-helpers-home.md#L157), [lines 221-228](docs/plans/2026-09-24-test-helpers-home.md#L221)). The destination module explicitly defines its responsibility as fake shapes for the two streamed LLM dialects ([`llm_sdk.py`, lines 1-17](vinga-server/tests/support/llm_sdk.py#L1)); the new helper instead constructs a real SDK client for ASR and TTS mock transports.
