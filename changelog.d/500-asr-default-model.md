...
- **The `openai` ASR type transcribes with `gpt-transcribe` by default**
  ...
  the conversation record start carrying a language they never carried
  on the old default. It is also billed differently: OpenAI publishes it
  at $0.0045 per minute of audio, where the model it replaces is priced
  per audio token, so what a transcription costs is now readable from
  the milliseconds the `asr` span already reports. Set
  `model: gpt-4o-mini-transcribe` on the entry to keep exactly what you
  had.
