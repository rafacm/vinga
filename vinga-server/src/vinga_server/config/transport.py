...
The policy lives in a module of its own because two callers apply it,
on opposite sides of the connection and each of them in both
directions. The repository asks it of every fragment it parses and of
every row it reads back; the CLI asks it of a fragment before that
fragment travels as a request body, and of an answer before that answer
is written as a machine-readable document. Those are the places a
mistake can still be met by a sentence rather than by a stack trace.
One rule and one wording, so whichever of them sees the value first
says the same thing about it. The alternative is what the encoder does
on its own: a TypeError, a ValueError or a RecursionError with a
traceback, in place of the sentence this file exists to produce.
